...
void decode(const string& aphb, char** HC){
    ...
    int match_num = num;
    int start_let_code_index = -1; // 字母的编码在code中开始匹配的位置
        ...
        if(match_num == 1){
            for(int match_index = 1; match_index <= num; match_index++){
                if(match[match_index] == 1){
                    let_code_len = strlen(HC[match_index]);
            ...
            let_code_index = 0;
            match_num = num;
            code_index = start_let_code_index + let_code_len;
...
int main(){
    ...
    HuffmanTree HT;
    int num = aphb.length();
    // TEST(26个字母的权重): 64 13 22 32 103 21 15 47 57 1 5 32 20 57 63 15 1 48 51 80 23 8 18 1 16 1 168
